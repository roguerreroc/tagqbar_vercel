...
export default async function PublicEtiquetaPage({ params }: PageProps) {
  ...
  // Look up etiqueta by token (UUID)
  ...
  supabaseAdmin
    ...
    .insert([{
      ...
      detalles: `Visualización de etiqueta #${etiqueta.id}`,
      ...
      <div className="w-full max-w-md bg-white rounded-[2rem] shadow-2xl overflow-hidden border border-slate-100 relative transition-all-smooth hover-lift">
        ...
        {etiqueta.estado === 'activa' && currentActivacion ? (
          <div className="p-8">
            ...
            <div className="grid grid-cols-2 gap-y-6 gap-x-4 mb-4">
              ...
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                ...
                <p className="text-sm font-bold text-slate-800 font-mono">#{etiqueta.id}</p>
              </div>
            </div>
          ...
          <div className="p-12 text-center bg-slate-50">
             ...
             <p className="text-slate-500 text-sm leading-relaxed max-w-[250px] mx-auto">Esta etiqueta no está asignada a ningún vuelo activo actualmente o su validez ya expiró.</p>
          </div>
        )}
